          ...
          >
            ...
            <div className="flex items-center justify-between px-5 pt-4 pb-3 border-b border-white/5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-black text-white">Odds Calculator</span>
              </div>
              <button
            ...
            <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              <div className="space-y-2">
                <div className="text-[9px] font-black text-slate-600 uppercase tracking-widest">Odds Input</div>
                {bets.map((bet, idx) => (
                  <div key={bet.id} className="flex items-center gap-2">
                    <input
                      type="text"
                      placeholder={`Bet ${idx + 1}`}
                      value={bet.label}
              ...
              {/* 배팅금 */}
              <div>
                <div className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-1.5">Stake</div>
              ...
              <div className="bg-black/20 rounded-2xl p-4 border border-white/5 space-y-3">
                <div className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-2">Result</div>
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-black text-slate-500">Total Odds</span>
                  <span className="text-sm font-black text-white tabular-nums">
                    {totalOdds !== null ? totalOdds.toFixed(3) : '–'}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-black text-slate-500">Implied Prob.</span>
                  <span className="text-sm font-black text-indigo-400 tabular-nums">{impliedProb}</span>
                </div>
                <div className="h-px bg-white/5" />
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-black text-slate-500">Payout</span>
                  <span className="text-base font-black text-emerald-400 tabular-nums">
                    {payout !== null ? `$ ${formatNum(payout)}` : '–'}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-black text-slate-500">Profit</span>
